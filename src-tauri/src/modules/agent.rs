...
pub fn agent_claude_hooks_status() -> bool {
    ...
    HOOK_EVENTS
        .iter()
        .all(|(_, m)| content.contains(&format!("notify;JavaRf;{m}")))
}
...
mod tests {
    ...
    fn adds_all_event_hooks_to_empty_config() {
        ...
        assert!(command(&out, "Notification", 0).contains("notify;JavaRf;attention"));
        assert!(command(&out, "Stop", 0).contains("notify;JavaRf;finished"));
        assert!(command(&out, "UserPromptSubmit", 0).contains("notify;JavaRf;working"));
        ...
        let legacy = json!({
            "hooks": {
                "Notification": [
                    { "hooks": [ {
                        "type": "command",
                        "command": "[ -n \"$JAVARF_TERMINAL\" ] && printf '\\033]777;javarf;notify\\033\\\\' > /dev/tty || true"
                    } ] }
                ]
            }
        });
