...
fn generate_sentinel() -> String {
    ...
    format!("__JAVARF_CWD_{:016x}_{:016x}__", mix, counter)
}
...
fn wrap_with_sentinel(command: &str, workspace: &WorkspaceEnv, sentinel: &str) -> String {
    ...
    {
        format!(
        "{command}\n$__javarf_rc = if ($null -ne $LASTEXITCODE) {{ $LASTEXITCODE }} elseif ($?) {{ 0 }} else {{ 1 }}\n\"`n{sentinel}$($PWD.Path)\"\nexit $__javarf_rc\n",
    )
    }
}
...
mod tests {
    ...
    fn sentinels_are_unique_per_session() {
        ...
        assert!(a.sentinel.starts_with("__JAVARF_CWD_"));
    ...
    fn strip_uses_session_sentinel_only() {
        ...
        let attacker = "__JAVARF_CWD_0000000000000000_0000000000000000__/evil";
        let trailer = format!("\n{}/real\n", s.sentinel);
    ...
    fn strip_returns_none_when_session_sentinel_absent() {
        let s = ShellSession::new("/tmp".into(), WorkspaceEnv::Local);
        let stdout = "some output\n__JAVARF_CWD_aaaa_bbbb__/spoof\nmore\n";
