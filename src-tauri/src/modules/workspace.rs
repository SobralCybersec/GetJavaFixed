...
mod auth_tests {
    ...
    fn tempdir(label: &str) -> PathBuf {
        ...
        let nanos = std::time::SystemTime::now()
            ...
            .map(|d| d.as_nanos())
            .unwrap_or(0);
        p.push(format!("javarf-auth-{label}-{nanos}-{}", std::process::id()));
    ...
    fn authorize_user_spawn_cwd_rejects_missing_path() {
        let mut missing = env::temp_dir();
        missing.push(format!("javarf-user-missing-{}", std::process::id()));
        let reg = WorkspaceRegistry::default();
        let s = missing.to_string_lossy().into_owned();
    ...
    fn resolve_launch_cwd_ignores_nonexistent_cli_dir() {
        ...
        let resolved = resolve_launch_cwd(Some("/no/such/javarf/dir"), Some(env.clone()));
