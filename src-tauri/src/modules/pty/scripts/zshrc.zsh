# javarf-shell-integration (zshrc)
#
...
# zsh, so we shadow $? into `_javarf_ret`.

{
  _javarf_user_zdotdir="${JAVARF_USER_ZDOTDIR:-$HOME}"
  [ -f "$_javarf_user_zdotdir/.zshrc" ] && source "$_javarf_user_zdotdir/.zshrc"
...
if [[ -z "$__JAVARF_HOOKS_LOADED" ]]; then
  __JAVARF_HOOKS_LOADED=1
  ...
  _javarf_precmd() {
    local _javarf_ret=$?
    printf '\e]133;D;%s\e\\' "$_javarf_ret"
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_javarf_urlencode "$PWD")"
  ...
  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _javarf_precmd
    add-zsh-hook preexec _javarf_preexec
  fi
