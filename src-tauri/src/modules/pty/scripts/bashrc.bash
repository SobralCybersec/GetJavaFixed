# javarf-shell-integration (bashrc)
#
...
if [ -z "$__JAVARF_HOOKS_LOADED" ]; then
  ...
  _javarf_precmd() {
    ...
    printf '\e]7;file://%s%s\e\\' "${HOSTNAME:-$(uname -n 2>/dev/null)}" "$(_javarf_urlencode "$PWD")"
    if [ -z "$__JAVARF_PS1_INJECTED" ]; then
      PS1='\[\e]133;B\e\\\]'"$PS1"
      __JAVARF_PS1_INJECTED=1
    fi
    printf '\e]133;A\e\\'
  }
  ...
  case ":${PROMPT_COMMAND:-}:" in
    *":_javarf_precmd:"*) ;;
    *) PROMPT_COMMAND="_javarf_precmd${PROMPT_COMMAND:+;$PROMPT_COMMAND}" ;;
  esac
