...
mod tests {
    ...
    #[test]
    fn javarf_marker_drives_status() {
        let mut d = AgentDetector::new();
        run(&mut d, &osc("133;C;claude"));
        assert_eq!(run(&mut d, &osc("777;notify;JavaRf;attention")), vec![Transition::Attention]);
        assert_eq!(run(&mut d, &osc("777;notify;JavaRf;working")), vec![Transition::Working]);
        assert!(run(&mut d, &osc("777;notify;JavaRf;working")).is_empty());
        assert_eq!(run(&mut d, &osc("777;notify;JavaRf;finished")), vec![Transition::Finished]);
    }

    #[test]
    fn javarf_marker_auto_arms_without_preexec() {
        let mut d = AgentDetector::new();
        assert_eq!(
            run(&mut d, &osc("777;notify;JavaRf;attention")),
