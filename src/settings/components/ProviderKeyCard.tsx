...
export function ProviderKeyCard({
  ...
  const submit = async () => {
    ...
    if (!trimmed) {
      setError(t("models.enterApiKey"));
      return;
    }
    if (provider.keyPrefix && !trimmed.startsWith(provider.keyPrefix)) {
      setError(t("models.keyPrefix", { provider: provider.label, prefix: provider.keyPrefix }));
      return;
    }
    setSaving(true);
    setError(null);
    try {
      ...
      setError(t("models.saveFailed", { error: String(e) }));
    } finally {
      setSaving(false);
    }
  };
