...
const INIT_PROMPT = `Scan this workspace and produce REFACTOR.md at the workspace root with:
...
Use grep/glob/list_directory/read_file to explore. Cap REFACTOR.md under 200 lines. Use write_file to create it (will go through normal approval).`;
...
export const JAVARF_CMD_RE =
  /^<javarf-command\s+name="([a-z0-9-]+)"(?:\s+state="([a-z]+)")?\s*\/>(?:\n+|$)/;

export function wrapWithCommandMarker(prompt: string, name: string): string {
  return `<javarf-command name="${name}" />\n\n${prompt}`;
}
