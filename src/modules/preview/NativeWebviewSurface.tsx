...
type WebviewUrlPayload = {
  label: string;
  url: string;
};

export function NativeWebviewSurface({
  label,
  url,
  visible,
  onUrlChange,
  ...
  useEffect(() => {
    ...
    async function syncWebview() {
      ...
      await (visible ? showNativeWebview(label) : hideNativeWebview(label)).catch(
        () => undefined,
      );
    }

    void syncWebview().catch((err) => {
      if (!disposed) setError(String(err || t("preview.nativeFailed")));
    });

    return () => {
      disposed = true;
    };
  }, [label, readBounds, syncBounds, t, url, visible]);
  ...
  useEffect(() => {
    ...
    void getCurrentWebviewWindow()
      .listen<WebviewUrlPayload>("javarf:preview-webview-url", (event) => {
  ...
  return (
    <div ref={hostRef} className="relative h-full min-h-0 w-full min-w-0 overflow-hidden bg-background">
      <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
        <div className="rounded-md border border-border/70 bg-card/90 px-3 py-2 text-xs text-muted-foreground shadow-sm">
          {error ?? t("preview.nativeActive")}
        </div>
      </div>
    </div>
  );
